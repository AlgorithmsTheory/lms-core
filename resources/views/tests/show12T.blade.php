...
<h4><b>Эмулятор Тьюринга</b></h4>
<h4>Тестов прошло: {{$choice['sequences_true']}} / {{$choice['sequences_all']}}</h4>
<h4>Количество отладок (нажатий "Проверить работу"): {{$choice['debug_counter']}}</h4>
<h4>Количество проверок синтаксиса: {{$choice['check_syntax_counter']}}</h4>
<h4>Количество запусков: {{$choice['run_counter']}}</h4>
<h4>Процент верно пройденных тестов: {{$choice['right_percent']}}%</h4>
<h4>Штраф за отладки, проверки синтаксиса и запуски составил: {{$choice['fee_percent']}}%</h4>
