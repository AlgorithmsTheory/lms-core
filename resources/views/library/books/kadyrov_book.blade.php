...
<div class="col-lg-6">
    <div class="row">
        ...
        <div class="col-lg-6">
            <a  href = "{{ action ('BooksController@kadyrov_getBook', [$book->id]) }}">
{!! HTML::image('img/library/'.$book->coverImg, 'book', array('style' => 'border-color: transparent; float:left; height:280px; width:200px;'))!!}
            </a>

</div>
<div class="col-lg-6 ">
<a href= "{{ action ('BooksController@kadyrov_getBook', [$book->id]) }}"><h3 class="text-left" >{{$book->title}}</h3></a>
<h4 class="text-left" >{{$book->author}}</h4>
    ...
    @if($role == 'Админ' )
        ...
        {!! HTML::link('Kadyrov/library/book/'.$book->id.'/edit','Редактировать книгу',array('class' => 'btn ink-reaction btn-primary btn-sm btn-block','role' => 'button')) !!}
