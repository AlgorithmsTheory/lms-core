...
public function open_MT(){
    ...
    $start_date_tur = DB::select("SELECT start_date FROM kontr_rab WHERE id = '1' AND NOW() > start_date AND NOW() < finish_date");
 ...
 public function open_HAM(){
    ...
    $start_date_nam = DB::select("SELECT start_date FROM kontr_rab WHERE id = '2' AND NOW() > start_date AND NOW() < finish_date");
...
//        return "0101";
    ...
    function runHAM($word, $rule_file){
        $cmd = "/usr/local/bin/normal.sh";
        $task_file = tempnam(sys_get_temp_dir(), 'norm_t_'); //создание файла с уникальным именем
        $this->add_to_file($task_file, $word);
        $res = exec($cmd . " " . $rule_file . " " . $task_file); //запуск эмулятура
//        unlink($task_file);  //удаление файла
        $res = json_decode($res, True);
...
public function kontr_HAMPOST(Request $request) {
    ...
    $input = array();
        ...
        if ( $res = $sqnc) {
           foreach ($res as $row ) {
                $n_tests += 1;
                $result = $this->runHAM($row['input_word'], $rule_file);
                if ( $result == $row['conv'] ) {
                    $mark+= 1;
                }
...
////        echo(json_encode($repost));
        ...
        // $variant = DB::select("SELECT variant FROM tasks WHERE id_task =".$id_task);
       
        //   if ($task_number == 1)
        //     {
        //         DB::update( "UPDATE  user_result_tur SET id_task_1='$id_task',mark_1='$result_mark',user_time_1='00:00:01'");
        //     }
        //      else 
        //     {
        //         DB::update( "UPDATE  user_result_tur SET  id_task_2='$id_task',mark_2='$result_mark',user_time_2='00:00:01'");
        //     }  
...
//         DB::insert( "INSERT INTO user_result_tur (id_user,variant) VALUES ('1','$variant')");
...
public function get_control_tasks_nam(Request $request){
       ...
       //записать вариант в БД на студента
        // DB::insert( "INSERT INTO user_result_nam (id_user,variant) VALUES ('1','$variant')");
        $res = EmulatorController::magic($res);
        foreach ($res as $r){
            array_push ( $array, $r);
        }
...
//Создание протокола
    ...
    public function get_HAM_protocol(Request $request){
        if ($request->ajax()) {
            $protocol = new HAMProtocol($request->input('id'), $request->input('id_user'), $request->input('html_text'));
            $protocol->create();
            return;
        }
    }
