...
class StatementsController extends Controller{
    ...
    private function showStudentAccount($user) {
        ...
        return view('personal_account/student_account',  compact('course_plan','statement_lecture','statement_seminar', 'statement_result', 'user'));
    }

    public function getStudentsByGroup(Request $request) {
        $data = $this->getData($request);
        $groupId = $data->groupId;
        $users = User::where([['group', '=', $groupId], ['role', '=', 'Студент']])->join('groups', 'groups.group_id', '=', 'users.group')
