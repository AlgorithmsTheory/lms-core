...
function run_one_normal_2(j){
	var step = j;
	var task = new Object()
	task.rule = new Array()
	task.str = 'Λ'+$('div.active >* textarea[name=textarea_src2]').val()
	...
	$.ajax({
		...
		success: function(data){
			...
			if ( resp.error != 'ok' ) {
				...
				$('input[id=onesend_2]').val("Ошибка!");
				if (step == true){
					debag(resp);
				}
			} else {
				$('input[id=onesend_2]').val(resp.result);
				if (step == true){
					debag(resp);
				}
			}
